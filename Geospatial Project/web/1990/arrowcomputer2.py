...
def load_matrices():
    reader = csv.reader(open('final_1990.csv', 'rU'), delimiter=',')
    row = reader.next()
    state_codes  = row[2:]
    i=0
    matrix = {}
    ...
    f = open("name_to_code_1990.json",'w')
...
def create_country_json(country_code,matrix,ccode_to_coordinates,file_prefix="in"):
    ...
    f = open('generated_1990/'+file_prefix+country_code+".json","w")
    print '/generated_1990/'+file_prefix+country_code+".json"
    f.write(json.dumps(res))
    f.close()


geoloc_data = json.loads(open('geoloc_1990.json').read())
...
f = open("code_to_coordinates_1990.json",'w')
